...
class TTrust_Text extends WP_Widget {

	function TTrust_Text() {
		global $ttrust_theme_name, $ttrust_version, $options;
		$widget_ops = array('classname' => 'ttrust_widget', 'description' => 'Custom text widget Make it wide or narrow.');
		$this->WP_Widget('ttrust_text_wide', $ttrust_theme_name.' '.__('Text Box', 'themetrust'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if($size=="wide") $c = ' class="twoThird '; else $c = ' class="oneThird ';
		
		$before_widget = str_replace('class="', $c , $before_widget);
		echo $before_widget;
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('icon'); ?>"><?php _e('Size:', 'themetrust'); ?></label>
		<select class="widefat" id="<?php echo $this->get_field_id('size'); ?>" name="<?php echo $this->get_field_name('size'); ?>">			
			<option value="narrow" <?php selected("narrow", $size) ?>><?php _e('Narrow', 'themetrust'); ?></option>
			<option value="wide" <?php selected("wide", $size) ?>><?php _e('Wide', 'themetrust'); ?></option>			
		</select></p>
<?php
	}
}

register_widget('TTrust_Text');
...
?>		
		...
		<?php if($show_post == "true") :?>
			...
			<?php $ttrust_feed = $rp_cat ? get_category_feed_link($rp_cat, '') : ttrust_get_option('ttrust_rss'); ?>
				...
				<?php if($i==1) :?>
				<div class="firstPost">
					<span class="meta"><?php the_time(get_option('date_format')); ?> <?php if($rp_cat == "") : ?> in <?php the_category(', '); ?> <?php endif; ?></span>
		...
		<?php else : ?>	
			...
			<?php $before_widget = str_replace('class="', 'class="oneFourth ' , $before_widget); ?>
			<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
...
<?php
	...
	function form( $instance ) {
		...
		<label for="<?php echo $this->get_field_id('show_post'); ?>"><?php _e('Show latest post', 'themetrust'); ?></label><br /><small>(This option is only recommended for footers and home page content area.)</small></p>
